import { Card } from "../components/ui/card";
import { Button } from "../components/ui/button";
import { Heart, MapPin, Users, CheckCircle } from "lucide-react";
import { motion } from "motion/react";
import { toast } from "sonner";

const necesidades = [
  {
    id: "1",
    hogar: "Los Girasoles",
    tipo: "Materiales Didácticos",
    descripcion: "Se necesitan libros de cuentos y material de motricidad fina",
    prioridad: "Media",
    fecha: "2026-04-05",
  },
  {
    id: "2",
    hogar: "Las Margaritas",
    tipo: "Alimentos",
    descripcion: "Frutas frescas y verduras para complementar nutrición",
    prioridad: "Alta",
    fecha: "2026-04-07",
  },
  {
    id: "3",
    hogar: "Los Girasoles",
    tipo: "Equipamiento",
    descripcion: "Juguetes educativos para desarrollo cognitivo",
    prioridad: "Baja",
    fecha: "2026-04-03",
  },
];

const prioridadStyles: Record<string, { border: string; bg: string; badge: string }> = {
  Alta:  { border: "border-red-500",    bg: "bg-red-50",    badge: "bg-red-200 text-red-900" },
  Media: { border: "border-orange-500", bg: "bg-orange-50", badge: "bg-orange-200 text-orange-900" },
  Baja:  { border: "border-blue-500",   bg: "bg-blue-50",   badge: "bg-blue-200 text-blue-900" },
};

export default function VoluntariadoVoluntarios() {
  const handleAyudar = (id: string) => {
    toast.success("¡Gracias! Un trabajador social te contactará pronto", { duration: 4000 });
  };

  return (
    <div className="space-y-6">
      {/* Header */}
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-center gap-3 mb-2">
          <Users className="w-8 h-8 text-purple-600" />
          <h2 className="text-3xl font-bold">Voluntariado — Voluntarios</h2>
        </div>
        <p className="text-gray-600">Explora las necesidades activas y ofrece tu ayuda a los hogares comunitarios</p>
      </motion.div>

      {/* Cómo funciona */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
        <Card className="p-6 bg-gradient-to-r from-purple-50 to-blue-50 border-purple-200">
          <div className="flex items-start gap-4">
            <div className="w-14 h-14 bg-purple-100 rounded-full flex items-center justify-center flex-shrink-0">
              <Users className="w-7 h-7 text-purple-600" />
            </div>
            <div>
              <h3 className="font-bold text-lg mb-2 text-purple-900">¿Cómo ser voluntario?</h3>
              <p className="text-gray-700 text-sm">
                Revisa las necesidades publicadas por las madres comunitarias, elige la que más
                se ajuste a lo que puedes ofrecer y regístra tu interés. Un trabajador social
                del ICBF te guiará en el proceso de entrega.
              </p>
              <div className="flex flex-wrap gap-4 mt-4">
                {["Proceso seguro y verificado", "Impacto directo en los niños", "Acompañamiento del ICBF"].map((t) => (
                  <div key={t} className="flex items-center gap-2 text-sm text-gray-700">
                    <CheckCircle className="w-4 h-4 text-purple-600" />
                    <span>{t}</span>
                  </div>
                ))}
              </div>
            </div>
          </div>
        </Card>
      </motion.div>

      {/* Pasos */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
        <Card className="p-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              { step: "1", title: "Explora", desc: "Revisa las necesidades activas de los hogares comunitarios", color: "bg-blue-100 text-blue-700" },
              { step: "2", title: "Elige",   desc: "Selecciona la necesidad con la que deseas contribuir",       color: "bg-purple-100 text-purple-700" },
              { step: "3", title: "Ayuda",   desc: "Un trabajador social del ICBF te guiará en el proceso",     color: "bg-green-100 text-green-700" },
            ].map((item) => (
              <div key={item.step} className="flex gap-3 items-start">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 font-bold text-sm ${item.color}`}>
                  {item.step}
                </div>
                <div>
                  <p className="font-medium text-sm">{item.title}</p>
                  <p className="text-xs text-gray-600 mt-1">{item.desc}</p>
                </div>
              </div>
            ))}
          </div>
        </Card>
      </motion.div>

      {/* Necesidades activas */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
        <div className="flex items-center gap-2 mb-4">
          <Heart className="w-5 h-5 text-gray-600" />
          <h3 className="text-xl font-semibold">Necesidades Activas</h3>
        </div>
        <div className="space-y-4">
          {necesidades.map((n, idx) => {
            const s = prioridadStyles[n.prioridad];
            return (
              <motion.div
                key={n.id}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.2 + idx * 0.1 }}
              >
                <Card className={`p-4 border-l-4 ${s.border} ${s.bg}`}>
                  <div className="space-y-3">
                    <div className="flex items-start justify-between">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <MapPin className="w-4 h-4 text-gray-600" />
                          <h4 className="font-semibold">{n.hogar}</h4>
                        </div>
                        <p className="text-sm text-gray-600">{n.tipo}</p>
                      </div>
                      <span className={`px-2 py-1 rounded text-xs font-medium ${s.badge}`}>
                        {n.prioridad}
                      </span>
                    </div>
                    <p className="text-gray-700">{n.descripcion}</p>
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-gray-500">Publicado: {n.fecha}</span>
                      <Button
                        size="sm"
                        onClick={() => handleAyudar(n.id)}
                        className="gap-2 bg-purple-600 hover:bg-purple-700"
                      >
                        <Heart className="w-4 h-4" />
                        Quiero Ayudar
                      </Button>
                    </div>
                  </div>
                </Card>
              </motion.div>
            );
          })}
        </div>
      </motion.div>
    </div>
  );
}
